...
import { IconMessageCircle, IconClock, IconUser, IconArrowUpRight, IconInfoCircle } from "@tabler/icons-react"
        ...
        <div className="flex items-center gap-2 min-w-0">
          {issue.assignee?.avatar ? (
            <img 
              ...
              alt={issue.assignee.name || 'Asignado'} 
      ...
      {viewComments && issue.lastComment && (
        <div className="mt-4 pt-4 border-t border-gray-100">
            ...
            <div className="flex items-center gap-2">
              {issue.lastComment.author?.avatar ? (
                <img 
                  ...
                  alt={issue.lastComment.author.name || 'Autor'} 
              ...
              <span className="text-sm font-medium text-gray-700">
                {issue.lastComment.author?.name || 'Usuario'}
              </span>
            </div>
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-sm text-gray-700 max-h-32 overflow-y-auto">
            {issue.lastComment.content && Array.isArray(issue.lastComment.content) ? (
              issue.lastComment.content.map((paragraph, index) => (
                <p key={index} className="mb-2 last:mb-0">
                  {String(paragraph)}
                </p>
              ))
            ) : (
              <p>{String(issue.lastComment.content)}</p>
            )}
          </div>
        </div>
      )}
        ...
        <div className="mt-4 pt-4 border-t border-gray-100 text-sm">
          <div className="space-y-2">
            <div className="flex items-center gap-2">
              ...
              <span className="text-gray-600">{formatDate(issue.created)}</span>
            </div>
            {issue.reporter && (
              <div className="flex items-center gap-2">
